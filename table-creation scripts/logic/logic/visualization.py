...
class Visualization:
    ...
    def __init__(self):
        if os.path.exists('../output/wb.xlsx'):
            os.remove('../output/wb.xlsx')

        self.workbook = Workbook()

        # these year names will appear in the table
        group_data_path = '../data/groups_schedules'
        years = os.listdir(path=group_data_path)

        # these file paths are used to fetch groups_schedules for groups_schedules
        groups = [[f'{group_data_path}/{year_dir}/{group_file}'
                   for group_file in os.listdir(f'{group_data_path}/{year_dir}')] for year_dir in years]

        # there are 3 columns of groups_schedules for each day entry
        self.day_data_width = 3

        # these colors are used for courses
        styles = pd.read_csv('../data/styles.csv', header=None, index_col=0)
        # print(styles)
        ...
        # save the notebook
        self.workbook.save(filename='../output/wb.xlsx')
    ...
    def add_group(self, group_column, group_data_path, palette):
        ...
        filename = os.path.split(group_data_path)
        group_name = os.path.splitext(os.path.basename(group_data_path))[0]
        ...
        df = pd.read_csv(f'{group_data_path}', index_col=0)
    ...
    def create_left(self, path_to_sample_data, palette):
        ...
        df = pd.read_csv(path_to_sample_data, index_col=0)
